...
cron.schedule('0 * * * *', async () => {
  ...
  try {
    ...
    for (const session of upcomingSessions) {
      ...
      console.log(
        `Đã gửi nhắc nhở cho buổi tập của khách hàng ${client.username}`
      );
    }
  } catch (error) {
...
cron.schedule('0 18 * * 0', async () => {
  // Mỗi tuần vào Chủ Nhật lúc 18:00
  console.log('Đang xử lý tổng kết tuần...');
...
async function generateWeeklySummaryNotifications() {
  try {
    ...
    for (const plan of workoutPlans) {
      ...
      console.log(
        `Đã cập nhật summary_generated cho WorkoutPlan ID: ${workout_plan_id}`
      );
    }
  } catch (error) {
